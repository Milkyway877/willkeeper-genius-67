...
WITH CHECK (bucket_id = 'will_documents' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Policy to allow users to view their own documents
CREATE POLICY "Users can view their own documents"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'will_documents' AND (storage.foldername(name))[1] = auth.uid()::text);
...
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'will_videos' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Policy to allow users to view their own videos
CREATE POLICY "Users can view their own videos"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'will_videos' AND (storage.foldername(name))[1] = auth.uid()::text);
...
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'will_images' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Policy to allow users to view their own images
CREATE POLICY "Users can view their own images"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'will_images' AND (storage.foldername(name))[1] = auth.uid()::text);
