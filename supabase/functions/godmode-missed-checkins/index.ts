...
serve(async (req) => {
  ...
  try {
    const { action, userId, force } = await req.json() as GodModeRequest;
...
async function scanAndProcessAllMissedCheckins() {
  ...
  if (!missedUsers || missedUsers.length === 0) {
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "No missed check-ins found",
        processed: 0 
      }),
...
async function processUserMissedCheckin(userId: string, force: boolean = false) {
  ...
  // Check if we've already sent notifications recently (within last 24 hours) unless forced
...
async function sendUserReminder(user: any, daysOverdue: number, urgencyLevel: string) {
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Your WillTank check-in is now <strong>${daysOverdue} days overdue</strong>. This automated system has detected your missed check-in and is taking action to protect your digital legacy.</p>
...
async function sendContactNotifications(userId: string, userFullName: string, userEmail: string, daysOverdue: number, urgencyLevel: string) {
  ...
  // Compile all contacts with their roles
  ...
  // Get primary executor for emergency contact info
      ...
      const emailContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f3f4f6; border: 1px solid #d1d5db; padding: 15px; border-radius: 4px; margin: 20px 0;">
            <h4 style="color: #374151; margin-top: 0;">📋 Your Contact Information on File</h4>
            <p style="color: #6b7280; margin: 0;">
              <strong>Role:</strong> ${contact.type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}<br>
              ${contact.relation ? `<strong>Relationship:</strong> ${contact.relation}<br>` : ''}
              <strong>Contact Method:</strong> ${contact.email}
            </p>
          </div>
