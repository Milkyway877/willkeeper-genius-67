...
This document is a preview generated by WillTank's AI system. It is not a legal document and would require customization, verification, and proper execution to be valid.
  ...
  const onUserDetailsSubmit = async (data: UserDetailsInputs) => {
    setUserDetails(data);
    setStep(2);
  };
  ...
  const onTanKeySubmit = (key: string) => {
    setTanKey(key);
    setStep(4);
  };
  
  const onRecoveryPhraseSubmit = async (data: RecoveryPhraseInputs) => {
    try {
      ...
      // Update the user's recovery phrase
      const { error } = await supabase
        .from("users")
        .update({ recovery_phrase: recoveryPhrase.join(' ') })
        .eq("id", user.id);
      ...
      if (error) {
        ...
        toast({
          title: "Warning",
          description: "We couldn't save your recovery phrase, but you can continue.",
          variant: "warning"
        });
      } else {
        toast({
          title: "Recovery phrase saved",
          description: "Your recovery phrase has been securely stored.",
        });
      }
      
      setStep(5);
    } catch (error) {
      console.error("Error saving recovery phrase:", error);
      toast({
        title: "Error",
        description: "Could not save your recovery phrase. You can still continue.",
        variant: "warning",
      });
      // Continue anyway
      setStep(5);
  ...
  return (
    <div className="w-full">
      ...
      {step === 3 && <TanKeyStep onNext={onTanKeySubmit} />}
