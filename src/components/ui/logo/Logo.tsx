...
export function Logo({ size = 'md', color = 'primary', className }: LogoProps) {
  ...
  const getColorClasses = () => {
    switch (color) {
      ...
      case 'primary':
      default: return 'text-willtank-500';
    }
  };

  return (
    <div className={cn('flex items-center gap-2 font-bold', getSizeClasses(), getColorClasses(), className)}>
      <div className="relative">
        <div className="absolute inset-0 animate-pulse-soft blur-sm bg-willtank-400 rounded-full"></div>
        <svg
          className="relative"
          width="24"
          height="24"
      ...
      <span className="tracking-tight">WillTank</span>
    </div>
  );
}
