...
export const FreemiumLayout: React.FC<FreemiumLayoutProps> = ({ children }) => {
  ...
  // Show grace period indicator if user has content in grace period
  const gracePeriodContent = expiredContent.gracePeriodContent;
  const hasGracePeriodContent = gracePeriodContent.length > 0;
  const shortestTimeRemaining = Math.min(...gracePeriodContent.map(c => c.expires_in_hours));
