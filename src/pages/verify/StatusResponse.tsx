  ...
  useEffect(() => {
    const processResponse = async () => {
      try {
        ...
        if (response !== 'alive' && response !== 'deceased') {
          setResult({
            success: false,
        ...
        const result = await processVerificationResponse(token, response);
        setResult(result);
    ...
    <Layout>
      <div className="max-w-md mx-auto mt-10">
        <Card className="border-2">
          <CardHeader>
            <CardTitle className="text-center">Status Check Response</CardTitle>
